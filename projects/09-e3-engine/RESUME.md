...
2. **Stage J is all but done.** Six units built, all regenerated by
   ...
   pack and J6 `POST.md`. **The only Stage J item left is the rotating
   cutaway on the site**, deliberately not built from this side because
   `app/turbofan/` already renders the E³ procedurally and a second
   three.js page would duplicate it.
