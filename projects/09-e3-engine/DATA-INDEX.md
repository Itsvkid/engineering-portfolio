...
| **Thrust-bearing axial loads, MEASURED** — `data/icls-thrust-bearing.yaml`: No. 3 (HP) forward, flat at 3.1 kN to 75 % Nc then 44.5 kN at 94.4 %; No. 1 (LP) aft, reaching −82.8 kN at 98.3 % Nf; sign convention printed on p.525; the printed pound and kN axes close to 0.048 kN. **No bearing capacity, bore or DN is printed in any source**, and that absence is recorded in the same file with the five documents searched. Corrects unit E4's 2026-09-07 gate — finding 269 | **T** (measured) | CR-168211 §6.17.1 p.525, Figs 340–341 pp.527–528 (PDF 552–553); `tools/read_icls_thrust_bearing.py`, `tests/test_thrust_bearing.py`; closure E4 |
...
| **Fan rotor blade — DESIGNED, not transcribed** — `solvers/blading/fan_blade.py`: camber and stagger at 10 radii from the island to near the tip, from Fig 3's radial pressure profile, the published Mach numbers, chord, thickness, count and 5° incidence, with Carter deviation. Camber 35° → 1°, turning 13° → 54°, tip solidity 1.43 vs a printed 1.40. **Throat margin 11–17 % against a published 5–8.8 %** — the one number not used to build it. Outboard of ~90 % span the designed camber falls under 1° and the arc construction is undefined | **DESIGN** | C3 unit 15, `tests/test_fan_blade_design.py`; the E³ never published this blade |
| **Rotor 37 validation case** — `data/methods/rotor37-validation-case.yaml`: TP-1337's design point and measured 100 % speed line, the published Glenn-HT CFD result, and **the pass bands stated before any solver exists** (1 % choking flow, 0.02 pressure ratio, 1.5 efficiency points, 3° exit angle, GCI 3 %). Four scan-corrupted efficiencies recorded null and recomputed from the printed ratios rather than transcribed; the three clean ones recompute to 0.005 | **T** | TP-1337 Tables I, V(a); CR-2010-216235; C4 step 0, `tests/test_rotor37_case.py` |
| **HPT cooling effectiveness, all four rows** — `solvers/thermal/cooling.py`: φ from four printed temperatures per row, no model. **φ/(1−φ) = 0.421·Wc^0.916, R² = 0.98** over coolant flows of 0.76–6.30 % of W25 and gas temperatures 1038–1739 °C; the exponent is what Dittus–Boelter predicts. Metal held to 26 °C across the four. The stage-2 vane's apparent 24 % outlier is a **station mismatch** — it is printed at 95 % span, and at 65 % span it falls on the line to 1.9 % | **D** | derived; Stage D unit D1, `tests/test_cooling_effectiveness.py` |
| **Stage-1 blade chordwise metal temperature** — `solvers/thermal/cooling.py`: Fig 23's external heat-transfer coefficient plus **one** fitted internal conductance (5,476 W/m²·°C) reproduces Fig 27 at the suction surface (−3 K) and midchord (−8 K) against a ±25 K band; the leading edge misses +31 K and the deficit is a film effectiveness of 0.064 there and ≈0 elsewhere — where the three showerhead rows are | **D** | derived; Stage D unit D2, `tests/test_blade_chordwise_temperature.py` |
| **Secondary-air network, checked** — `solvers/thermal/secondary_air.py`: the eight Table VII items sum to exactly 18.87 % of W25 and the four Table XI streams to **16.14 %**; both stage-1 nozzle cavities keep hot gas out. The printed backflow margin reproduces against the gas **total** for the forward cavity (1.46 vs 1.45) and only against the gas **static** for the aft (1.00 vs 1.00, and 0.32 against total) — recorded as printed | **D** | derived; Stage D unit D3, `tests/test_secondary_air.py` |
| **ACC payoff and the cruise clearance, two routes** — `solvers/thermal/clearance.py`: Table X recomputes line by line to 0.006 point and 0.002 on the total (1.535 vs 1.533 % efficiency), sfc closing exactly at −1.22 %. **Table III's tip clearance in % of span and §4's desired clearance in cm are the same number**: 0.0427 vs 0.041 (stage 1) and 0.0419 vs 0.041 (stage 2) — 0.040 % and 0.013 % of span apart | **D** | derived; Stage D unit D4, `tests/test_clearance_control.py` |
| **Combustor exit profile and pattern factor** — `solvers/thermal/combustor.py`: Fig 8's 24 airflow labels summing to exactly 100.0 % (40.6 % domes, 59.4 % cooling and dilution); and the finding that **T41 is the rotor inlet, not the combustor exit** — used as the exit average it gives PF 0.386 against a 0.25 requirement, while the noted 0.26 implies an exit of 1503 °C, **82 °C above T41**, the nonchargeable coolant's worth, agreeing with Stage B's independent 55 K at a milder condition | **D** | derived; Stage D unit D5, `tests/test_combustor_exit.py` |
| **HPC blade root centrifugal stress** — `solvers/mechanical/blade_stress.py`: all ten stages reproduced within **6.5 %** of Table X from Table XXII's area distribution at the deteriorated-engine speed. **The blade material crossover is an output**: stages 1–4 titanium (within 6.5 %), 5–10 nickel (within 5.5 %), neither working for the other group — the split lands on the inertia weld CR-168219 describes. Taper factors 0.56–0.80 | **D** | derived; Stage E unit E1, `tests/test_blade_centrifugal_stress.py` |
| **HPT disc, rotating-disc model** — `solvers/mechanical/disc.py`: bore doubling exact (2.0000 as a/b → 0); rim load 5,882 kN (600 tonnes) on the stage-1 disc; published bores 779–910 MPa bracketed by 691 (solid) and 1388 (small hole) MPa. **Screen of all nineteen Fig. 55 locations against N²: none passes** — the rotor is thermal at its limiting times. The Fig. 64 peak is **not** here: it needs the disc profile, still un-digitised | **D** | derived; Stage E unit E2, `tests/test_disc_stress.py` |
| **Blade natural frequencies** — `solvers/mechanical/beam.py` + `blade_frequency.py`: Euler–Bernoulli FE exact to 0.0001 % on three closed-form boundary conditions; section properties by Green's theorem. Booster rotor **−2.7 %** of a published 250 Hz; fan 80 Hz inside its bracket; HPC stage-9/10 vanes inside theirs at a consistent 0.62–0.66 restraint fraction. **Ten HPC rotor first-three-mode predictions recorded against Figs 33–42, which remain un-transcribed** | **D** | derived; Stage E unit E3, `tests/test_blade_frequency.py` |
| **HPC rotor Campbell diagrams** — `data/hpc-rotor-campbell.yaml`: all ten (Figs 33–42), read off the page images with the reading uncertainty stated. **Lifts the gate that stood on E3's closure since Stage A**; the closure is now evaluated and fails at 1 of 24 inside its 5 % band | **A** | transcribed; Stage E unit E3, `tests/test_blade_frequency.py` |
| **HPT stage-1 disc stress, Fig. 64** — `data/hpt-mechanical.yaml`: effective stresses at eight points on the disc plus the dovetail detail, 40 s into accel. Peak 1138 MPa at the forward arm; **the bore reads 1034 MPa where Fig. 55's CLASS/MASS model gives 779 at the same instant, a 33 % model-to-model difference** | **A** | transcribed; Stage E unit E2 |
| **Shafts, criticals and blade-out** — `solvers/mechanical/rotordynamics.py`: HP/LP shaft torque at three ratings (LP 68.6 kNm vs HP 28.7 at takeoff); spool speeds closing across three documents to 0.5 %; Table XXII's four margins recomputed to 0.008; the aft seal disc's implied mode stiffening S = 8.20 from Fig. 88; fan blade-out 746 kN (76 tonnes); airfoil mass 66 % / 48 % of Table VI's per-blade weights. **Bearing loads and capacities are not here: the reports print none** | **D** | derived; Stage E unit E4, `tests/test_rotordynamics.py` |
| **Attachments and joints** — `solvers/mechanical/attachments.py`: HPT two-tang load split recovered at 57/43 from the printed stress-to-width ratio (0.47 %); the printed combined stress is 6.4× the nominal neck tension; fan/booster crush implied over 1.88 and 0.98 flanks; LPT Fig. 70's printed Kt reproduced to 0.6 % on the blade sections and not on the disc; three LPT retainers on one allowable with the load law recovered as F/t² on t2; casing flange bolt stress at the 2× criterion. **HPC §3.2.3 dovetails are not here: nothing was transcribed** | **D** | derived; Stage E unit E5, `tests/test_attachments.py` |
...
| **Sensitivity** — `solvers/verification/sensitivity.py`: one-at-a-time elasticities of sfc, HPT blade metal temperature and disc bore stress, every input labelled published / measured / handbook. Six have exact analytic values and are used as the step-0 validation | **D** | derived; Stage I unit I2, `tests/test_sensitivity.py` |
| **Ranked disagreements** — `solvers/verification/disagreements.py` and `FINDINGS.md`: all 98 predicted-vs-published comparisons, ranked, each with a source and a cause or `unresolved` | **D** | derived; Stage I unit I3, `tests/test_findings.py` |
| **Exact Riemann solution** — `solvers/cfd/shocktube.py`: Toro's exact solver written out, returning Sod's textbook p*/p_L = 0.30313 to five figures, used as the known answer for the CFD install. OpenFOAM v2406 reproduces it to **0.02 % on p\***, 0.13 % on u\*, and 0.20 cells on the shock position | **D** | derived; Stage C4 unit C4-1, `tests/test_shocktube.py` |
| **NASA Rotor 37 blade coordinates** — `data/methods/rotor37-blade-coordinates.yaml`: twelve sections hub to tip from TP-1337 Appendix C, read off the page images because the scan's OCR is unusable. Plus the flow path from figure 1(a), datumed on the blade hub leading edge. Verified by 24 exact closure constraints, monotonic stagger, non-negative thickness, and a tip speed matching Table I to −0.00 % | **A** | transcribed; Stage C4 unit C4-2, `tests/test_rotor37_geometry.py` |
...
| **HPT live-disc rims and the two drawings cross-checked** — stage-1 rim (dovetail seat) **30.768 cm**, stage-2 **27.884 cm**, so 1.69 and 3.29 cm of shank-plus-dovetail; three near-cylindrical surfaces at the bottom of the rotor at r 6.9–8.3; two bolt-joint centrelines at r ≈ 20.5 and 17.1. Fig 50 calibrated on the **same four published flowpath anchors** as Fig 112, to **0.027 cm rms** at 62.32 px/cm with a +0.543° page rotation off the caption type. **The five common radii disagree between the two figures by 1.5 % at r 20 and 14 % at r 7 — all inside the combined 95 % interval, so consistent and imprecise, not wrong** — `data/hpt-fig50-calibration.yaml` | **A** | **HPT report Fig. 50 p.87 (PDF page 96), "E³ HPT Major Design Features" — drawn over the published flowpath, undimensioned**, against Fig. 112 p.180; `tools/read_hpt_fig50.py`, `solvers/mechanical/hpt_disc.py`, `tests/test_hpt_fig50.py`, E14 |
| **HPT stage-2 vane cooling and purge pressures, MEASURED** — the stage-7 feed, the interstage cavity and the **forward and aft wheelspace pressures** against HPT corrected speed, psi and kPa axes; text says no discernible difference between the cavity and the forward wheelspace, backflow margins 2.6–4.4 kPa (0.39–1 % Δp), never negative. **The forward and aft wheelspaces are the cavities on the stage-1 disc's aft face and the stage-2 disc's forward face — two of the four faces D6 item 2 needs. Not digitised; the named next step for D6** | **T** (measured) | **CR-168211 Fig. 210 p.348 (PDF page 373), text p.343**; finding 288 — corrects unit E14's own step 0 |
