...
*Status 2026-09-07 — the solver route is now open.* `colima` is installed
with a stopped `default` profile (aarch64, 4 CPU, 6 GiB, 30 GiB, docker
runtime), so a Docker daemon needs `colima start` and **not** Docker
Desktop. **SU2 is not on conda-forge** — `conda search -c conda-forge su2`
returns no match on this platform, so that route does not exist and the
OpenFOAM container is the one that does.
...
- [x] Blade rows from C3 sections — **unit G1**,
      `solvers/geometry/blades.py`. **All 32 rows** (fan, booster, 10 HPC
      rotors, 10 HPC stators, 10 LPT rows) build as **valid solids**, and
      the CAD volume matches Stage F2's integral of the same sections to
      **0.94 % worst** (finding 115). PF-06 is extended with the
      **conformal cylindrical wrap**, because PF-06's own stated validity
      condition — "chord is small next to radius" — fails here at
      chord/radius 0.49 on HPC rotor 1
- [ ] Discs from E2 profiles; shafts from E4; blade attachments —
      **GATED** on the un-digitised disc profiles (finding 117)
- [ ] Flowpath surfaces, casings, liner, dome from A3 and D2 — **GATED**:
      A3 figure-status
- [ ] Nacelle from CST fitted to Fig. 40 — PF-07 — **GATED**: Fig. 40 is
      figure-status
- [x] Compound STEP, bodies grouped by spool (`lp`, `hp`, `static`) — 35
      files, 132 MB, regenerated by `python build.py --export`. glTF is
...
- [x] `build.py`: one command regenerates every table, figure and export.
      **29 of 29 solver modules run in 90 s**, each one's output written to
      ...
      quoted in the STEP0 files

*Closes when:* `python build.py` on a clean clone reproduces everything,
and the generated mass matches F2. **Substantially closed 2026-09-07**:
`build.py` runs, and the generated blade set *is* the geometry F2 weighed,
to 0.94 %. There is no engine mass to match because F2's own total is
gated. Four of the seven bullets are blocked on transcription rather than
on method (finding 117) — the machinery that built the blades is the same
machinery a disc or a casing needs, and it demonstrably works.
...
- [x] Does it run and export STEP? **The "corrupt install" note was wrong.**
      Checked 2026-09-07: `~/Applications/Autodesk Fusion.app` is the
      168 kB launcher stub Fusion always installs, and the real payload is
      present and complete — **11 GB** under
      `~/Library/Application Support/Autodesk/webdeploy/production/`, with
      its own `Autodesk Fusion.app` inside. Version 2704.1.53. The stub is
      unsigned, which is normal for it. **Launching it and exporting one
      STEP is the only thing left to confirm, and that needs a human at
      the GUI.** Stage H is not gated on an install
...
- [x] The same T41, cooling flows, metal temperatures and masses across
      stages — **unit I1**, `solvers/verification/consistency.py`,
      `tests/test_cross_discipline.py`. **T41 at the aero design point:
      0.01 %** between the cycle model and the HPT report's own cycle-match
      line. **The four cooling streams: 0.00 %** — the cycle's bleeds *are*
      the streams D3 measured. Masses F2 vs G1: 0.94 % over 32 rows. The
      HPT report's *earlier requirements* block is chronology and not
      disagreement, and comparing against it would have manufactured a
      finding out of a design history (finding 119)
- [x] Spool speeds from four routes agree — LP **1.31 %** (fan corrected
      tip speed vs LPT N/√T), HP **1.55 %** (HPC Table X vs HPT N/√T), and
      the ratio falls out at **3.57 against the plan's own 3.6** for
      Stage H's assembly motion (finding 121)
- [x] Every "closes when" tolerance re-checked — `data/closures.yaml`
      carries all 20 with their bands and today's numbers. **14 of 15
      numeric closures are inside their band**; the one miss is B3's
      takeoff sfc, a pinned xfail with its cause recorded. 7 met, 10 half,
      3 gated, and **no closure is open without a reason attached**
      (finding 122)
...
- [x] Every disagreement with a published number, ranked by size, with a
      cause or "unresolved" — **`FINDINGS.md`**, generated by
      `tools/build_findings.py` from the solvers so it cannot drift.
      **98 comparisons, median 6.7 %**, 56 inside 10 %, and **5 recorded
      unresolved** (findings 146–148)
- [x] Every place the E³ *design* (FPS) and the E³ *as tested* (ICLS)
      differ, and which the model matches — the ICLS burned **2.5 % above
      its own prediction** and the report accounts for all of it in six
      items summing to exactly 2.5. This model is built to the FPS design
      and sits with the design, not the test (finding 149)
...
- [x] Meridional plot **first**; Campbell match second; render third
      ...
      unpublished offsets drawn as labelled gaps. Findings 155–158.
      Campbell match and render still open.
...
**A3 is the project's binding constraint.** Every gate recorded in Stages
E, F and G traces back to it: the HPT disc cross-sections (E2's peak
stress, the 120 % burst margin, F2's disc masses, G's disc bodies), the HPC
rotor Campbell diagrams Figs 33–42 (E3's closure), the HPC blade and
...
target, but its closure also asks that *every* cavity has a pressure that
keeps hot gas out, and only the stage-1 nozzle's two are done. D4 is the
one Stage D box that is fully closed, and it says so in its own status
line.
